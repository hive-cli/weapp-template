...
var Utils = {
  ...
  getDistance(latFrom, lngFrom, latTo, lngTo) {
    ...
    var distance =
      ...
      Math.asin(
        Math.sqrt(
          Math.pow(Math.sin(a / 2), 2) + Math.cos(radLatFrom) * Math.cos(radLatTo) * Math.pow(Math.sin(b / 2), 2)
        )
      )
  ...
  handleData(param, data, feature) {
    if (feature == 'search') {
      ...
      var reverseGeocoderSimplify = {
        ...
        street_number:
          (reverseGeocoderResult.address_component && reverseGeocoderResult.address_component.street_number) || null,
        recommend:
          (reverseGeocoderResult.formatted_addresses && reverseGeocoderResult.formatted_addresses.recommend) || null,
    ...
    var locationsuccess = function (result) {
      if (region && !rectangle) {
        requestParam.boundary =
          'region(' + region + ',' + auto_extend + ',' + result.latitude + ',' + result.longitude + ')'
        if (options.sig) {
        ...
        requestParam.boundary =
          'nearby(' + result.latitude + ',' + result.longitude + ',' + distance + ',' + auto_extend + ')'
        if (options.sig) {
          requestParam.sig = Utils.getSig(requestParam, options.sig, 'search')
        }
      }
